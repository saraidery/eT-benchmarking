...
def get_data_for_molecule(
    molecule, basis, folder, search_strings, methods, timing_file
):
    #
...
def print_latex_table(
    search_strings, methods, data_wall, data_cpu, caption, summary_file
):
    #
...
def get_summary(folder, summary_file, title, timings_file_name):
    ...
    for molecule in sorted(molecules):
        ...
        for basis in sorted(bases):
            ...
            data_wall_t, data_cpu_t = get_data_for_molecule(
                molecule,
                basis,
                basis_folder,
                search_strings,
...
get_summary(
    "disco",
    "disco.tex",
    "Disco timing benchmark on (small) bigmem nodes on Saga",
    "eT.timing.out",
)
